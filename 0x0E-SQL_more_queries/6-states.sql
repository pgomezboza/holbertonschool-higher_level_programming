-- creates the db "hbtn_0d_usa" and the table "states" (in the db hbtn_0d_usa).
-- states description:
-- 1. id INT unique, auto generated, can’t be null and is a primary key.
-- 2. name VARCHAR(256) can’t be null.
-- 3. If the database "hbtn_0d_usa" already exists, your script should not fail.
-- 4. If the table "states" already exists, your script should not fail.
CREATE DATABASE IF NOT EXISTS hbtn_0d_usa;
USE hbtn_0d_usa;
CREATE TABLE IF NOT EXISTS states (id INT NOT NULL UNIQUE AUTO_INCREMENT, name VARCHAR(256) NOT NULL, PRIMARY KEY(id));
